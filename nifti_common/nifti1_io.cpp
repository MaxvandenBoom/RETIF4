...
static char * gni_history[] = 
{
  ...
  "     (problem found by Atle Bjшrnerud)\n"
...
char *nifti_strdup(const char *str)
{
  ...
  /* check for failure */
  //if( dup ) strcpy_s(dup, str);
  if( dup ) strcpy_s(dup, strlen(str) + 1, str);
...
char * nifti_find_file_extension( const char * name )
{
   ...
   //strcpy_s(extcopy, ext);
   strcpy_s(extcopy, sizeof(extcopy), ext);
   ...
   strcat_s(elist[0], extgz); strcat_s(elist[1], extgz); strcat_s(elist[2], extgz);
...
char * nifti_findhdrname(const char* fname)
{
   ...
   //strcpy_s(hdrname,basename);
   strcpy_s(hdrname, strlen(basename)+8, basename);
   //strcat_s(hdrname,elist[efirst]);
   strcat_s(hdrname, strlen(basename)+8, elist[efirst]);
   if (nifti_fileexists(hdrname)) { free(basename); return hdrname; }
#ifdef HAVE_ZLIB
   strcat_s(hdrname,extzip); 
   if (nifti_fileexists(hdrname)) { free(basename); return hdrname; }
#endif
...
char * nifti_findimgname(const char* fname , int nifti_type)
{
   ...
   if( nifti_type == NIFTI_FTYPE_ASCII ){
      //strcpy_s(imgname,basename);
	  strcpy_s(imgname, strlen(basename)+8, basename);
      //strcat_s(imgname,extnia);
	  strcat_s(imgname, strlen(basename)+8, extnia);
      ...
      //strcpy_s(imgname,basename);
	  strcpy_s(imgname, strlen(basename)+8, basename);
      //strcat_s(imgname,elist[first]);
	  strcat_s(imgname, strlen(basename)+8, elist[first]);
      if (nifti_fileexists(imgname)) { free(basename); return imgname; }
#ifdef HAVE_ZLIB  /* then also check for .gz */
      strcat_s(imgname,extzip);
      if (nifti_fileexists(imgname)) { free(basename); return imgname; }
#endif
      ...
      //strcpy_s(imgname,basename);
	  strcpy_s(imgname, strlen(basename)+8, basename);
      //strcat_s(imgname,elist[1-first]);  /* can do this with only 2 choices */
	  strcat_s(imgname, strlen(basename)+8, elist[1-first]);  /* can do this with only 2 choices */
      if (nifti_fileexists(imgname)) { free(basename); return imgname; }
#ifdef HAVE_ZLIB  /* then also check for .gz */
      strcat_s(imgname,extzip);
      if (nifti_fileexists(imgname)) { free(basename); return imgname; }
                         ...
                         int comp)
{
   ...
   //strcpy_s(iname, prefix);
   strcpy_s(iname, strlen(prefix)+8, prefix);
   ...
   /* otherwise, make one up */
	   ...
	   //strcat_s(iname, extnii);
	   strcat_s(iname, strlen(prefix)+8, extnii);
   else if( nifti_type == NIFTI_FTYPE_ASCII )
	   //strcat_s(iname, extnia);
	   strcat_s(iname, strlen(prefix)+8, extnia);
   else                                          
	   //strcat_s(iname, exthdr);
	   strcat_s(iname, strlen(prefix)+8, exthdr);

#ifdef HAVE_ZLIB  /* if compression is requested, make sure of suffix */
   if( comp && (!ext || !strstr(iname,extgz)) ) strcat_s(iname,extgz);
#endif
                         ...
                         int comp)
{
   ...
   /* otherwise, make one up */
   else if( nifti_type == NIFTI_FTYPE_NIFTI1_1 ) strcat_s(iname, strlen(prefix)+8, extnii);
   else if( nifti_type == NIFTI_FTYPE_ASCII )    strcat_s(iname, strlen(prefix)+8, extnia);
   else                                          strcat_s(iname, strlen(prefix)+8, extimg);
...
nifti_image* nifti_simple_init_nim(void)
{
  ...
  int nbyper, swapsize;
   ...
   strcpy_s(nhdr.magic, "n+1");  /* init to single file */
...
struct nifti_1_header nifti_convert_nim2nhdr(const nifti_image * nim)
{
   ...
   if( nim->nifti_type > NIFTI_FTYPE_ANALYZE ){ /* then not ANALYZE */
     ...
     if( nim->nifti_type == NIFTI_FTYPE_NIFTI1_1 ) strcpy_s(nhdr.magic,"n+1") ;
     else                                          strcpy_s(nhdr.magic,"ni1") ;
...
char *nifti_image_to_ascii( const nifti_image *nim )
{
   ...
   printf_s( buf , "<nifti_image\n" ) ;   /* XML-ish opener */

   printf_s( buf+strlen(buf) , "  nifti_type = '%s'\n" ,
   ...
   printf_s( buf+strlen(buf) , "  header_filename = %s\n",ebuf); free(ebuf);

   ebuf = escapize_string(nim->iname) ;
   printf_s( buf+strlen(buf) , "  image_filename = %s\n", ebuf); free(ebuf);

   printf_s( buf+strlen(buf) , "  image_offset = '%d'\n" , nim->iname_offset );

                       printf_s( buf+strlen(buf), "  ndim = '%d'\n", nim->ndim);
                       printf_s( buf+strlen(buf), "  nx = '%d'\n",   nim->nx  );
   if( nim->ndim > 1 ) printf_s( buf+strlen(buf), "  ny = '%d'\n",   nim->ny  );
   if( nim->ndim > 2 ) printf_s( buf+strlen(buf), "  nz = '%d'\n",   nim->nz  );
   if( nim->ndim > 3 ) printf_s( buf+strlen(buf), "  nt = '%d'\n",   nim->nt  );
   if( nim->ndim > 4 ) printf_s( buf+strlen(buf), "  nu = '%d'\n",   nim->nu  );
   if( nim->ndim > 5 ) printf_s( buf+strlen(buf), "  nv = '%d'\n",   nim->nv  );
   if( nim->ndim > 6 ) printf_s( buf+strlen(buf), "  nw = '%d'\n",   nim->nw  );
                       printf_s( buf+strlen(buf), "  dx = '%g'\n",   nim->dx  );
   if( nim->ndim > 1 ) printf_s( buf+strlen(buf), "  dy = '%g'\n",   nim->dy  );
   if( nim->ndim > 2 ) printf_s( buf+strlen(buf), "  dz = '%g'\n",   nim->dz  );
   if( nim->ndim > 3 ) printf_s( buf+strlen(buf), "  dt = '%g'\n",   nim->dt  );
   if( nim->ndim > 4 ) printf_s( buf+strlen(buf), "  du = '%g'\n",   nim->du  );
   if( nim->ndim > 5 ) printf_s( buf+strlen(buf), "  dv = '%g'\n",   nim->dv  );
   if( nim->ndim > 6 ) printf_s( buf+strlen(buf), "  dw = '%g'\n",   nim->dw  );

   printf_s( buf+strlen(buf) , "  datatype = '%d'\n" , nim->datatype ) ;
   printf_s( buf+strlen(buf) , "  datatype_name = '%s'\n" ,
                              nifti_datatype_string(nim->datatype) ) ;

   printf_s( buf+strlen(buf) , "  nvox = '%u'\n" , (unsigned)nim->nvox ) ;
   printf_s( buf+strlen(buf) , "  nbyper = '%d'\n" , nim->nbyper ) ;

   printf_s( buf+strlen(buf) , "  byteorder = '%s'\n" ,
            (nim->byteorder==MSB_FIRST) ? "MSB_FIRST" : "LSB_FIRST" ) ;

   if( nim->cal_min < nim->cal_max ){
     printf_s( buf+strlen(buf) , "  cal_min = '%g'\n", nim->cal_min ) ;
     printf_s( buf+strlen(buf) , "  cal_max = '%g'\n", nim->cal_max ) ;
   }

   if( nim->scl_slope != 0.0 ){
     printf_s( buf+strlen(buf) , "  scl_slope = '%g'\n" , nim->scl_slope ) ;
     printf_s( buf+strlen(buf) , "  scl_inter = '%g'\n" , nim->scl_inter ) ;
   }

   if( nim->intent_code > 0 ){
     printf_s( buf+strlen(buf) , "  intent_code = '%d'\n", nim->intent_code ) ;
     printf_s( buf+strlen(buf) , "  intent_code_name = '%s'\n" ,
                                nifti_intent_string(nim->intent_code) ) ;
     printf_s( buf+strlen(buf) , "  intent_p1 = '%g'\n" , nim->intent_p1 ) ;
     printf_s( buf+strlen(buf) , "  intent_p2 = '%g'\n" , nim->intent_p2 ) ;
     printf_s( buf+strlen(buf) , "  intent_p3 = '%g'\n" , nim->intent_p3 ) ;

     if( nim->intent_name[0] != '\0' ){
       ebuf = escapize_string(nim->intent_name) ;
       printf_s( buf+strlen(buf) , "  intent_name = %s\n",ebuf) ;
       free(ebuf) ;
     }
   }

   if( nim->toffset != 0.0 )
     printf_s( buf+strlen(buf) , "  toffset = '%g'\n",nim->toffset ) ;

   if( nim->xyz_units > 0 )
     printf_s( buf+strlen(buf) ,
              "  xyz_units = '%d'\n"
              "  xyz_units_name = '%s'\n" ,
   ...
   if( nim->freq_dim > 0 )
     printf_s( buf+strlen(buf) , "  freq_dim = '%d'\n",nim->freq_dim ) ;
   if( nim->phase_dim > 0 )
     printf_s( buf+strlen(buf) , "  phase_dim = '%d'\n",nim->phase_dim ) ;
   if( nim->slice_dim > 0 )
     printf_s( buf+strlen(buf) , "  slice_dim = '%d'\n",nim->slice_dim ) ;
   if( nim->slice_code > 0 )
     printf_s( buf+strlen(buf) ,
              "  slice_code = '%d'\n"
              "  slice_code_name = '%s'\n" ,
   ...
   if( nim->slice_duration != 0.0 )
     printf_s( buf+strlen(buf) , "  slice_duration = '%g'\n",
              nim->slice_duration ) ;

   if( nim->descrip[0] != '\0' ){
     ebuf = escapize_string(nim->descrip) ;
     printf_s( buf+strlen(buf) , "  descrip = %s\n",ebuf) ;
     free(ebuf) ;
   }

   if( nim->aux_file[0] != '\0' ){
     ebuf = escapize_string(nim->aux_file) ;
     printf_s( buf+strlen(buf) , "  aux_file = %s\n",ebuf) ;
     free(ebuf) ;
   }
   ...
   printf_s( buf+strlen(buf) , "  num_ext = '%d'\n", nim->num_ext ) ;
...
#define QSTR(nam,ml) if( strcmp(lhs,#nam) == 0 )                           \
                       strncpy_s(nim->nam,rhs,ml), nim->nam[ml]='\0'
...
nifti_image *nifti_image_from_ascii( const char *str, int * bytes_read )
{
   ...
   ii = sscanf_s( str+spos , "%1023s%n" , lhs , &nn ) ; spos += nn ;
   ...
   while(1){
     ...
     if( str[spos] == '\'' ){
        ...
        ii = sscanf_s( str+spos , "%1023s%n" , rhs , &nn ) ; spos += nn ;
     ...
     if( strcmp(lhs,"nifti_type") == 0 ){
       ...
       sscanf_s( rhs , "%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f" ,
               &(nim->sto_xyz.m[0][0]) , &(nim->sto_xyz.m[0][1]) ,
               &(nim->sto_xyz.m[0][2]) , &(nim->sto_xyz.m[0][3]) ,
               &(nim->sto_xyz.m[1][0]) , &(nim->sto_xyz.m[1][1]) ,
               &(nim->sto_xyz.m[1][2]) , &(nim->sto_xyz.m[1][3]) ,
