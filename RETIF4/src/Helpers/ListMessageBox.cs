...
namespace RETIF4.Helpers {
    ...
    public class ListMessageBox : Form {
        ...
        private Timer tmrFocus = new Timer();
        ...
        public ListMessageBox(string title, string[][] options, bool multiple) {
            ...
            this.Load += new EventHandler(delegate (object sender, EventArgs e) {
                ...
                // create a timer to hold this window in front
                tmrFocus.Interval = 50;
                tmrFocus.Tick += new EventHandler(delegate (object tmrSender, EventArgs tmrEvent) {
                    
                    // get the handle of the window currently on the foreground
                    IntPtr foregroundHandle = GetForegroundWindow();

                    // if focus is lost, then return focus to the form
                    if (foregroundHandle != this.Handle)
