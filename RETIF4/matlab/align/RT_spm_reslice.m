function [reslicedData, reslicedMat] = RT_spm_reslice(P)
    % P as coming from realign
    ...
    % variable to store all resliced data 
    % (is only used when more than 1 image is resliced)
    reslicedDataAll = [];
    if numel(P) > 2
        reslicedDataAll = zeros(P(1).dim(1), P(1).dim(2), P(1).dim(3), numel(P) - 1);
    end
    ...
    for i = 2:numel(P)
        ...
        if ~isfinite(flags.interp)
            reslicedData = abs(kspace3d(spm_bsplinc(P(i),[0 0 0 ; 0 0 0]'),P(1).mat\P(i).mat));
            for x3 = 1:P(1).dim(3)
                if flags.mask
        ...
        % if there is more than 1 image to reslice, then store all
        % resliced data in a seperate variable
        if numel(P) > 2
            reslicedDataAll(:, :, :, i - 1) = reslicedData;
        end    
        
    end

    % retrieve the matrix of the first image (to which all are resliced)
    reslicedMat = P(1).mat;

    % if there is more than 1 image to reslice, then return all the
    % resliced data from the seperate variable
    if numel(P) > 2
        reslicedData = reslicedDataAll;
    end
