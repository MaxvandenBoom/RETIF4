...
switch(par.ResToolsVersion)
    ...
    case {'V4','V4.1','V4.2'}
        ...
        if strcmp(par.ResToolsVersion,'V4.1') | strcmp(par.ResToolsVersion,'V4')
            slice_index=[slice_index,zeros(size(slice_index,1),2)]; % from V 4.2 and higher, par file slice index lines contain more info on diffusion gradients. For data versions below V4.2, add zeros. 
        end
        ...
        par.slice_index=slice_index(:,[1:11 43]); %last row is diffusion gradient number!
        if par.dyn>1
        ...
        if length(parameter)<15,
            ...
            par.rescale_slope=1/str2num(parameter{14});
