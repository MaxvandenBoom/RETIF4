...
%
%   Created by:
%   Bas Neggers, Dept of Brain Research, University Medical Center
%
%   Version 2.6.0, released 17-07-2006
global r2a_defaults;
...
dimlist=[3 4 5];
options.dim=dimlist(get(handles.dim,'value'));

if r2a_defaults.usedirtydtihack
    options.dti_revertb0=1; % a very dirty UMCU hack for DTI data, use with care
else
    options.dti_revertb0=0; % a very dirty UMCU hack for DTI data, use with care
end
...
if ~spmfound % no spm anywhere near (r2agui started in some scary place)
    disp('no known SPM version found, using default matlab file selector (yikes!)');
...
% Hints: contents = get(hObject,'String') returns dim contents as cell array
